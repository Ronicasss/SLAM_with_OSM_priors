...
public:
  ...
  virtual void onInit() {
    ...
    reset_counter = 0;
    dist_last_kf = 0.0;
    //pcl::console::setVerbosityLevel(pcl::console::L_DEBUG); // uncomment to get debug from pcl
        ...
        if(!keyframe->buildings_check && keyframe->index != 0) {
          ...
          *keyframe->odomCloud = *temp_cloud_5;
          ...
          if(new_buildings.size() > 0) {
            ...
            // vector containing all buildings nodes for current kf (new and not new)
            ...
            if(num_threads > 0) {
              if(private_nh.param<bool>("enable_ndt_num_threads", false))
                gicp->setNumThreads(private_nh.param<int>("ndt_num_threads", 0));
            }
            if(private_nh.param<bool>("enable_ndt_transformation_epsilon", false))
              gicp->setTransformationEpsilon(private_nh.param<double>("ndt_transformation_epsilon", 0.1));
            if(private_nh.param<bool>("enable_ndt_maximum_iterations", false))
              gicp->setMaximumIterations(private_nh.param<int>("ndt_maximum_iterations", 35));
            if(private_nh.param<bool>("enable_ndt_step_size", false))
              gicp->setStepSize(private_nh.param<double>("ndt_step_size", 0.1));
            if(private_nh.param<bool>("enable_ndt_outlier_ratio", false))
              gicp->setOulierRatio(private_nh.param<double>("ndt_outlier_ratio", 0.55));

            if(private_nh.param<bool>("enable_ndt_resolution", false))
              gicp->setResolution(private_nh.param<double>("ndt_resolution", 1.0));
            ...
            // old things about fixing the first node - may be removed
            if(fix_first_building) {
              ...
              // pcl_temp is a temporary cloud used to debug, currently unused
              ...
              // cut_dist indicates the maximum distance between two correspondences
              double cut_dist = 0.0;

              // you may take it from launcher or otherwise is automatically set to max_dist*max_dist
              ...
              // this function is called only to get back the correspondences src_corr and tgt_corr.
              // src_corr and tgt_corr, at the same index, contains the index of points in the pointclouds that correspond.
              // src_corr[x]=y and tgt_corr[x]=z means that src_pc[y] and tgt_pc[z] correspond.  
              ...
              for(auto it1 = bnodes.begin(); it1 != bnodes.end(); it1++)
              {
                ...
                for(int i = 0; i < tgt_corr->size(); i++) {
                  // since correspondences are on all buildings, the following for is needed to retrieve correspondences only related
                  // to the current building (whose pointcloud is b_pcl)  
                  ...
                  if(j != b_pcl->size()) {
                    // if the current correspondence (index i) is related to the current building enter the if.

                    // put both points of the correspondence in a temporary pointcloud (for visualization).
                ...
                // add the edge if there are enough inliers
                if(k > private_nh.param<int>("k_thresh", 0)) {
                  auto edge = graph_slam->add_se2_edge(bntemp->node, keyframe->node, t_s_b_iso.inverse(), inf);
                  graph_slam->add_robust_kernel(edge, private_nh.param<std::string>("map_edge_robust_kernel", "NONE"), private_nh.param<double>("map_edge_robust_kernel_size", 1.0));
                }
              }
              
              // publish the pcl_temp cloud in case it is used to debug
            ...
            // no buildings found in the surroundings of the current keyframe
