...
// includes for hdl_graph_slam_with_map_priors
...
private:
  ...
  bool flush_keyframe_queue() {
    ...
    for(int i = 0; i < std::min<int>(keyframe_queue.size(), max_keyframes_per_update); i++) {
      ...
      // information matrix calculator was left in 3D and then the information matrix is converted to 2D
    ...
    if(enter) { // test parameter
      for(auto& keyframe : keyframes) {
        ...
        if(!keyframe->buildings_check && keyframe->index != 0) {
          ...
          pcl::PointCloud<PointT3>::Ptr odomCloud(new pcl::PointCloud<PointT3>); 
          // temporary clouds
          ...
          // transform the final cloud from base_link frame to odom frame
          ...
          odomCloud->header.frame_id = "base_link";
           ...
           // publish original not processed keyframe cloud
          ...
          *keyframe->odomCloud = *temp_cloud_5;
          ...
          if(new_buildings.size() > 0) {
            ...
            std::vector<BuildingNode::Ptr> bnodes; 
            ...
            for(auto it2 = new_buildings.begin(); it2 != new_buildings.end(); it2++)
            {
              ...
              if(bntemp == nullptr) { // enter if the building is new
                ...
                // retrieve informations to build the se2 node
                // translation
